...
class SqlText
{
	...
	//$top(int)显示指定记录数
	static function select_mysql($cloumns,$table,$where,$order,$top)
	{
	...
	static function select_mysql_page($cloumns,$table,$where,$order,$pageindex,$pagesize)
	{
	...
	static function RecordPrev_mysql($columns, $table, $id, $value, $where=NULL)
	{
		if(!empty($where))
		{
			$where = $where." and";
	...
	//$value(string)ID字段值
	//$where(string)where查询
	static function RecordNext_mysql($columns, $table, $id, $value, $where=NULL)
	{
		if(!empty($where))
		{
			$where = $where." and";
	...
	//$pageindex(int)为当前页
	//$pagesize(int)为每页显示记录
	static function select_page($cloumns,$table,$where,$order,$pageindex,$pagesize)
	{
		if(!empty($top))
		{
			$sql.=" top ".$top;
